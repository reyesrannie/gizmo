...
const readExcelFileSupplier = async (files) => {
  if (files.length > 0) {
    ...
    const dataFive = [];
    worksheetFive.eachRow((row, rowNum) => {
      if (rowNum !== 1) {
        const rowData = {};
        row.eachCell((cell, colNum) => {
          rowData[headersFive[colNum]] = cell.value;
        });
        dataFive.push(rowData);
      }
    });
    ...
    const combinedData = dataOne.map((item) => {
      ...
      const matchedItemsFive = dataFive.filter((i) => i.tin === item.tin);

      const supplier_atcs = [];
      const supplier_types = [];
      const supplier_vats = [];
      const supplier_documenttypes = [];
      ...
      matchedItemsFive.forEach((i) => {
        supplier_documenttypes.push({
          id: i.id,
          code: i.code,
        });
      });
...
const readExcelFileDocumentType = async (files) => {
  if (files.length > 0) {
    const file = files[0];
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(file);
    ...
    // Combine data based on matching code and dept_code
    const combinedData = dataOne.map((item) => {
      const matchedItems = dataTwo.filter((i) => i.code === item.code);
      const required_fields = matchedItems.map((i) => i.field);
      return {
        code: item.code,
        name: item.name,
        required_fields,
